...
- [x] **CLI hint overlay (`PENDING`)** — bottom-right floating `Terminal` button on every dashboard view expands into a card with the equivalent `amaco …` commands for the current Route + run/task ID interpolation + flag tips (`--effort`, `--read-only`, `--provider`); pure `hintForRoute` in `src/ui/lib/cli-hints.ts` (5 new tests), `CliHintOverlay` component, wired in `App.tsx`.
